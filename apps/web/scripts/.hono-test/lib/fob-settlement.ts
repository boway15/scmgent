import type { AllocationMethod, CostStage } from './fob-fee-rules.js';
import type { ContainerMerchantStat } from './fob-container-stats.js';

export type MerchantVolume = {
  merchantCode: string;
...
export type FeeLine = {
  ...
  isException?: boolean;
  exceptionStatus?: 'pending' | 'confirmed' | 'rejected' | null;
};
...
export type AllocationRow = {
  ...
  stage: FeeLine['stage'];
  feeType: string;
  sourceBillType: FeeLine['sourceBillType'];
  sourceBillItemId: string;
  sourceRef?: string;
  allocationMethod: AllocationMethod;
  sourceAmountCny: number;
  merchantVolumeCbm: number;
  volumeRatio: number;
  ticketCount?: number;
  ticketRatio?: number;
...
export function compareAllocationRows<
  T extends { containerNo: string; feeType: string; id?: string },
>(a: T, b: T): number {
...
export function sortAllocationRows<T extends { containerNo: string; feeType: string; id?: string }>(
  rows: T[],
): T[] {
...
export function aggregateMerchantVolumes(
  rows: Array<{ merchantCode: string; merchantName?: string | null; containerNo: string; volumeCbm: number }>,
): Map<string, MerchantVolume[]> {
  const byContainer = new Map<string, Map<string, MerchantVolume>>();

  for (const row of rows) {
    if (!byContainer.has(row.containerNo)) byContainer.set(row.containerNo, new Map());
  ...
  const result = new Map<string, MerchantVolume[]>();
  for (const [container, merchants] of byContainer) {
    result.set(
      container,
      [...merchants.values()].sort((a, b) => a.merchantCode.localeCompare(b.merchantCode)),
    );
  }
  return result;
}

export function mergeVolumeAndTicketStats(
  volumeStats: Map<string, MerchantVolume[]>,
  ticketStats: Map<string, ContainerMerchantStat[]>,
): Map<string, MerchantVolume[]> {
  const result = new Map<string, MerchantVolume[]>();
  ...
  for (const containerNo of containers) {
    ...
    const merged = new Map<string, MerchantVolume>();
    ...
    for (const t of ticketRows) {
      ...
      if (existing) {
        ...
        if (!existing.merchantName && t.merchantName) existing.merchantName = t.merchantName;
      } else {
...
export function allocateFeesByVolume(
  merchants: MerchantVolume[],
  fee: FeeLine,
): AllocationRow[] {
...
export function allocateFeesByTicket(
  merchants: MerchantVolume[],
  fee: FeeLine,
): AllocationRow[] {
  const totalTickets = merchants.reduce((s, m) => s + (m.ticketCount ?? 0), 0);
  ...
  return splitAmount(
    merchants.filter((m) => (m.ticketCount ?? 0) > 0),
    fee,
    totalTickets,
    (m) => m.ticketCount ?? 0,
    (m, ratio, amount, isLast) => ({
      merchantVolumeCbm: m.volumeCbm,
...
export function allocateFeeSingleMerchant(merchant: MerchantVolume, fee: FeeLine): AllocationRow[] {
  return [
    {
...
export function allocateFeeFixed(
  merchants: MerchantVolume[],
  fee: FeeLine,
): AllocationRow[] {
...
export function allocateFeeManualAcrossMerchants(
  merchants: MerchantVolume[],
  fee: FeeLine,
  amountFor: (merchant: MerchantVolume) => number,
): AllocationRow[] {
...
/** 待异常确认：各主体默认 ¥0 */
export function allocateFeeManualPending(
  merchants: MerchantVolume[],
  fee: FeeLine,
): AllocationRow[] {
...
function allocationSum(rows: AllocationRow[]): number {
  return round2(rows.reduce((s, r) => s + r.allocatedAmountCny, 0));
}

/** 未平账或尚无分摊行时，需为柜内各主体补齐占位行（默认 ¥0） */
export function shouldPadMerchantPlaceholders(fee: FeeLine, rows: AllocationRow[]): boolean {
  if (!rows.length) return true;
  return Math.abs(fee.amountCny - allocationSum(rows)) > 0.01;
}

/** 为柜内缺失主体补 ¥0 行，便于未平账项在界面上直接调账 */
export function padMissingMerchantAllocations(
  merchants: MerchantVolume[],
  fee: FeeLine,
  rows: AllocationRow[],
): AllocationRow[] {
...
export function allocateFeeManual(
  merchants: MerchantVolume[],
  fee: FeeLine,
): AllocationRow[] {
  ...
  return allocateFeeManualAcrossMerchants(merchants, fee, (m) =>
    m.merchantCode === targetCode ? fee.amountCny : 0,
  );
}

export function allocateFees(
  merchantStatsByContainer: Map<string, MerchantVolume[]>,
  feeLines: FeeLine[],
): { allocations: AllocationRow[]; warnings: string[] } {
  const allocations: AllocationRow[] = [];
  const warnings: string[] = [];

  const feesByContainer = new Map<string, FeeLine[]>();
  for (const fee of feeLines) {
    if (!feesByContainer.has(fee.containerNo)) feesByContainer.set(fee.containerNo, []);
  ...
  for (const [containerNo, fees] of feesByContainer) {
    ...
    if (!merchants?.length) {
      warnings.push(`柜号 ${containerNo} 无商家体积/票数数据，已跳过 ${fees.length} 项费用`);
      continue;
    }
    ...
    for (const fee of fees) {
      ...
      if (method === 'manual') {
        ...
        if (!rows.length) {
          warnings.push(`柜号 ${containerNo} 费用 ${fee.feeType} 人工分摊缺少归属主体`);
        }
      } else if (method === 'fixed') {
        rows = allocateFeeFixed(merchants, fee);
        if (!rows.length) {
          warnings.push(`柜号 ${containerNo} 费用 ${fee.feeType} 固定费用需指定归属主体`);
        }
...
export function reconcileAllocations(
  ...
  const warnings: string[] = [];
  const byBillItem = new Map<string, AllocationRow[]>();

  for (const row of allocations) {
    const key = row.sourceBillItemId;
    if (!byBillItem.has(key)) byBillItem.set(key, []);
    byBillItem.get(key)!.push(row);
  }

  const containerChecks: ReconcileResult['containerChecks'] = [];
  let billTotalCny = 0;
...
export function summarizeByMerchant(allocations: AllocationRow[]) {
  const map = new Map<
    string,
    {
      merchantCode: string;
  ...
  for (const row of allocations) {
    ...
    if (!s.merchantName && row.merchantName) s.merchantName = row.merchantName;
    if (row.stage === 'trucking') s.truckingTotal += row.allocatedAmountCny;
    else if (row.stage === 'freight') s.freightTotal += row.allocatedAmountCny;
    else if (row.stage === 'customs') s.customsTotal += row.allocatedAmountCny;
    else s.otherTotal += row.allocatedAmountCny;
...
function splitAmount<T extends MerchantVolume>(
  merchants: T[],
  fee: FeeLine,
  totalBase: number,
  getBase: (m: T) => number,
