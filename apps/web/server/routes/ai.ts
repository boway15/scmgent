...
aiRoutes.post('/ai/chat', requireMenu('ai.chat'), async (c) => {
  ...
  let mode: 'dify' | 'local' | 'local-fallback' = difyConfig.difyEnabled ? 'dify' : 'local';
  let fallback = false;
  ...
  try {
    if (difyConfig.difyEnabled) {
      try {
        ...
        if (skuContext) difyInputs.sku_context = skuContext;
        if (body.skuCode?.trim()) difyInputs.sku_code = body.skuCode.trim();
        if (body.warehouseCode?.trim()) difyInputs.warehouse_code = body.warehouseCode.trim();
