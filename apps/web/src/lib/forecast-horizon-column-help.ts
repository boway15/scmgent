...
const V41 = {
  baseline:
    '混合水平（levelDaily）：锚定×(1−w)+季节朴素×w，w=min(0.62, 0.28+k×0.07)，持久化为 baseline_daily_avg。' +
    ' 是套限幅前的中间量；V4.1「基线」与「混合水平」同源。悬停单元格可看逐步计算。',
  ...
  seasonal:
    '季节朴素日均：截止上月末近 12 月销量序列，按地平线序号取同日历位月销量，再 ÷ 目标月天数。' +
    ' 悬停单元格可看算法步骤；远月混合时季节权重更高。',
  blendLevel:
    '混合水平 = 锚定×(1−w) + 季节朴素×w，w = min(0.62, 0.28 + k×0.07)，k 为地平线序号（首月 k=0）。' +
    ' 近端偏锚定，远端偏季节；悬停可看代入数值的逐步计算，再经趋势衰减与分层上下限得到「系统」列。',
} as const;
