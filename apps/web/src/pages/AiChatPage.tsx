...
export function AiChatPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      const content =
        data.fallback && !data.answer.includes('已切换本地助手')
    ...
    <div className="space-y-4">
      ...
      {config?.mode === 'local' && (
        <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-900">
          本地助手模式：基于供应链 FAQ 与业务数据摘要回答。配置{' '}
          <code className="text-xs">DIFY_API_KEY_KNOWLEDGE</code> 后将自动切换为知识库 RAG。
        </div>
      )}

      {config?.mode === 'dify' && (
        <div className="rounded-md border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm text-emerald-900">
          Dify 知识库模式：回答将基于 RAG 检索与传入的 SKU 业务上下文。
          {(config.replenishmentWorkflow || config.alertWorkflow) && (
            <span className="ml-2 text-emerald-800">
              已启用 Workflow：
              {config.replenishmentWorkflow ? ' 补货增强' : ''}
              {config.alertWorkflow ? ' 预警摘要' : ''}
            </span>
          )}
        </div>
      )}
            ...
            <div className="max-h-[420px] min-h-[280px] space-y-3 overflow-y-auto rounded-md border border-border bg-muted/20 p-4">
              {!messages.length && (
                <p className="text-sm text-text-hint">
                  可提问：安全库存如何计算？补货建议流程？PMC 计划如何下发？
                </p>
              )}
              {messages.map((m, i) => (
                <div
