...
const SECTIONS: HelpSection[] = [
  {
    ...
    body: (
      <>
        <p>
          本平台面向<strong>跨境家具供应链</strong>，覆盖商品主数据、多仓库存、安全库存与补货预测、PMC 下单计划、FOB 分账及本地 AI 助手。
        ...
        <table className="mt-3 w-full text-sm">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td className="p-2 text-text-sub">看板、库存、计划只读</td>
            </tr>
          </tbody>
        </table>
      </>
  ...
  },
  {
    ...
    body: (
      <p>
        ...
        配置 <code className="rounded bg-muted px-1">DIFY_API_KEY_KNOWLEDGE</code> 后切换 Dify RAG；
        补货/预警任务可分别配置 <code className="rounded bg-muted px-1">DIFY_API_KEY_REPLENISHMENT</code>、
        <code className="rounded bg-muted px-1">DIFY_API_KEY_ALERT</code> 增强文案。详见帮助文档或{' '}
        <code className="rounded bg-muted px-1">docs/dify/setup-guide.md</code>。
      </p>
    ),
  },
  {
    ...
    body: (
      <>
        <p>
          上线前访问 <code className="rounded bg-muted px-1">GET /api/health</code>，确认{' '}
          <code className="rounded bg-muted px-1">runtime.productionReady=true</code>，且{' '}
          <code className="rounded bg-muted px-1">authDevMode=false</code>。
        </p>
        <ul className="mt-2 list-disc space-y-1 pl-5 text-text-sub">
        ...
        <p className="mt-2 text-text-sub">
          完整清单见 <code className="rounded bg-muted px-1">docs/qa/p0-production-smoke-checklist.md</code>
        </p>
      </>
    ),
  },
