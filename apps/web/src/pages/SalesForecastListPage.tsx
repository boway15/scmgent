...
import { ProjectGroupSearchSelect } from '@/components/ProjectGroupSearchSelect';
import {
...
export function SalesForecastListPage() {
  ...
  const [generationProjectGroup, setGenerationProjectGroup] = useState('');
  ...
  const generateBaseline = useMutation({
    mutationFn: async () => {
      ...
      const initial = await api.generateSalesForecastBaseline({
        projectGroup: generationProjectGroup.trim() || undefined,
    ...
    onSuccess: async (data) => {
      ...
      const platformInfo = data.platformsGenerated?.length
          ...
          <Card className="border-primary/20 bg-primary/5">
            <CardContent className="space-y-3 py-3 text-sm">
              ...
              <p className="text-xs text-text-sub">
                每次「生成草稿」都会新建一条版本快照（名称含月数、范围），可在列表中并列对比——例如
                3 个月 vs 6 个月、全量 vs 项目组实验稿。复核满意后发布其中一条；旧草稿可保留或忽略。
              </p>
              <p className="text-xs text-text-sub">
          ...
          <Card>
            <CardHeader>
              ...
              <p className="mt-2 text-xs text-text-sub">
                项目组 / 单 SKU 生成会各自产生独立快照（或单 SKU 合并进目标草稿），便于实验对比；正式补货建议用「全平台」全量生成。
              </p>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex flex-wrap items-end gap-2">
                <label className="space-y-1 text-sm">
                  <span className="text-text-sub">项目组</span>
                  <ProjectGroupSearchSelect
                    value={generationProjectGroup}
                    onChange={(next) => {
                      setGenerationProjectGroup(next);
