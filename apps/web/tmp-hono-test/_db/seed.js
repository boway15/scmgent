...
import { roles, menus, roleMenus, users } from './schema/auth.js';
import { seedFobFeeRules } from './seed-fob-rules.js';
import { seedFobServiceProviders } from './seed-fob-service-providers.js';
const ROLE_SEEDS = [
...
async function removeMenuTreeByCode(code) {
...
async function removeDeprecatedMenus() {
    ...
    const sorted = [...DEPRECATED_MENU_CODES].sort((a, b) => b.split('.').length - a.split('.').length);
    for (const code of sorted) {
        await removeMenuTreeByCode(code);
    }
}
async function seedMenus(items, parentId) {
...
async function main() {
    ...
    for (const role of ROLE_SEEDS) {
        ...
        if (roleId)
            roleIdByCode.set(role.code, roleId);
    }
