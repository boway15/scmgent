import { extractFieldValue, listAllRecords, normalizeBitableKey, } from '../integrations/feishu-bitable.js';
import { BATCH_TRACKED_IMPORT_TYPES, buildImportPreviewResponse, } from './import/preview.js';
import { createImportBatch, finalizeImportBatch, } from './import/batch.js';
import { runImport } from './import/handlers.js';
const BITABLE_SYNC_TYPES = ['skus', 'inventory', 'sales'];
...
export const BITABLE_FIELD_MAPS = {
    skus: {
...
export function getBitableAppToken() {
    return process.env.FEISHU_BITABLE_APP_TOKEN?.trim() || undefined;
}
export function getBitableTableId(type) {
    const key = TABLE_ENV_KEYS[type];
    return process.env[key]?.trim() || undefined;
}
export function getBitableSyncConfig() {
...
export function isBitableSyncType(value) {
    return BITABLE_SYNC_TYPES.includes(value);
}
function lookupFieldValue(fields, alias) {
    if (alias in fields)
        return fields[alias];
    const normalizedAlias = normalizeBitableKey(alias);
    for (const [key, value] of Object.entries(fields)) {
        if (normalizeBitableKey(key) === normalizedAlias)
            return value;
    }
    return undefined;
}
export function mapBitableRecordToRow(record, type) {
...
export function isEmptyImportRow(row) {
    return !Object.values(row).some((v) => v.trim());
}
export function mapBitableRecordsToRows(records, type) {
    return records
        .map((record) => mapBitableRecordToRow(record, type))
        .filter((row) => !isEmptyImportRow(row));
}
export async function fetchMappedRows(type) {
    const appToken = getBitableAppToken();
    const tableId = getBitableTableId(type);
    if (!appToken || !tableId) {
        throw new Error(`Bitable sync not configured for ${type}. Set FEISHU_BITABLE_APP_TOKEN and ${TABLE_ENV_KEYS[type]}.`);
    }
    const records = await listAllRecords(appToken, tableId);
    return mapBitableRecordsToRows(records, type);
}
export function bitableSyncFileName(type) {
    const tableId = getBitableTableId(type) ?? 'unknown';
    return `feishu-bitable:${tableId}`;
}
export async function previewBitableSync(type) {
    const rows = await fetchMappedRows(type);
    const preview = await buildImportPreviewResponse(type, rows);
    return { ...preview, source: 'feishu-bitable' };
}
export async function executeBitableSync(type, userId) {
    const rows = await fetchMappedRows(type);
    const preview = await buildImportPreviewResponse(type, rows);
