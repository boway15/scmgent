...
export function cleanContainerNo(v) {
    return String(v ?? '')
        .replace(/\s+/g, '')
        .replace(/\n/g, '')
        .toUpperCase();
}
function normalizeFeeName(v) {
    return String(v ?? '')
        .replace(/\n/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
...
export function parseSenweiTruckingSheet(rows) {
    const errors = [];
    const items = [];
    let skippedRows = 0;
    if (rows.length < 3) {
...
export function isSenweiTruckingSheet(rows) {
    if (rows.length < 2)
        return false;
    const header = rows[1];
    return (findHeaderIndex(header, ['内部编号']) >= 0 &&
        findHeaderIndex(header, CONTAINER_HEADER_ALIASES) >= 0);
}
/** 识别简化拖车导入模板（表头第 1 行含「货柜号」+ 宽表费用列） */
export function isSimplifiedTruckingSheet(rows) {
    if (rows.length < 1)
        return false;
    const header = rows[0];
    return (findHeaderIndex(header, ['货柜号']) >= 0 &&
        findHeaderIndex(header, ['拖车费']) >= 0 &&
        findHeaderIndex(header, ['内部编号']) < 0);
}
/** 简化拖车导入：表头第 1 行，按列 unpivot 费用 */
export function parseSimplifiedTruckingSheet(rows) {
    const errors = [];
    const items = [];
    let skippedRows = 0;
    if (rows.length < 2) {
    ...
    header.forEach((name, i) => {
        ...
        if (!n || TRUCKING_META_HEADERS.has(String(name ?? '').trim()))
            return;
...
export function isHuamaoFreightSheet(rows) {
    if (rows.length < 6)
        return false;
    const header = rows[5];
...
export function isSimplifiedFreightSheet(rows) {
    if (rows.length < 1)
        return false;
    const header = rows[0];
    return (findHeaderIndex(header, ['货柜号']) >= 0 &&
...
export function parseSimplifiedFreightSheet(rows) {
    const errors = [];
    const items = [];
    let skippedRows = 0;
    if (rows.length < 2) {
    ...
    header.forEach((name, i) => {
        const n = normalizeFeeName(name);
        if (!n || FREIGHT_META_HEADERS.has(String(name ?? '').trim()))
            return;
        feeCols.push({ idx: i, name: n });
    });
    if (!feeCols.length) {
...
function mapHuamaoFeeStage(feeName) {
    if (/申报|关税|增值税|查验|清关/.test(feeName))
        return 'customs';
    return 'freight';
}
/** 华贸对账单：表头第 6 行；每行左右两栏各为独立票；金额均为人民币（业务侧已折算） */
export function parseHuamaoFreightSheet(rows) {
    const errors = [];
    const items = [];
    let skippedRows = 0;
    if (rows.length < 7) {
    ...
    header.forEach((name, i) => {
        ...
        if (!n || /合计/.test(n))
            return;
        if (i <= 24 && /\(USD\)|\(CNY\)/.test(n))
            leftFeeCols.push({ i, name: n });
        if (i >= 26 && /\(CNY\)/.test(n))
            rightFeeCols.push({ i, name: n });
    });
    const panels = [
...
export async function sheetRowsFromBuffer(buffer) {
...
const CONTAINER_HEADER_ALIASES = ['柜号', '货柜号', '箱号', '集装箱号', 'container_no', '临柜号'];
...
const VOLUME_HEADER_ALIASES = ['体积/m3', '体积m3', '体积', 'volume_cbm', 'volume'];
...
export function findEdVolumeHeaderRow(rows) {
    for (let i = 0; i < Math.min(rows.length, 10); i++) {
        ...
        const containerIdx = findHeaderIndex(header, CONTAINER_HEADER_ALIASES);
        if (factoryTypeIdx >= 0 && subjectIdx >= 0 && volumeIdx >= 0 && containerIdx >= 0)
            return i;
    }
    return -1;
}
/** 识别 ED 大件调拨导出（含 工厂类别 + 主体 + 体积列） */
export function isEdVolumeExport(rows) {
    return findEdVolumeHeaderRow(rows) >= 0;
}
function isFobFactoryType(v) {
    const s = String(v ?? '').trim();
    if (!s)
        return false;
    if (isNonFobOnlyFactoryType(s))
        return false;
    return /FOB/i.test(s);
}
function isNonFobOnlyFactoryType(v) {
    const s = String(v ?? '').trim();
    return /^非FOB$/i.test(s) || /不做FOB/i.test(s);
}
/** 货柜是否参与分账：含 FOB 行，或存在非「纯非FOB」的有效体积行（如退税/非退税） */
function containerQualifiesForAllocation(factoryTypes) {
    if (!factoryTypes.length)
        return false;
    if (factoryTypes.some((t) => isFobFactoryType(t)))
        return true;
    if (factoryTypes.every((t) => isNonFobOnlyFactoryType(t)))
        return false;
    return true;
}
export function findTransferVolumeHeaderRow(rows) {
    for (let i = 0; i < Math.min(rows.length, 10); i++) {
        const header = rows[i];
...
export function isTransferVolumeExport(rows) {
 ...
 * - 工厂类别取「是否退税」
 */
export function parseTransferVolumeSheet(rows) {
    const errors = [];
    const items = [];
    let skippedRows = 0;
    if (rows.length < 2) {
    ...
    const factoryTypesByContainer = new Map();
    const containersInFile = new Set();
    for (let r = headerRowIdx + 1; r < rows.length; r++) {
        ...
        if (!factoryTypesByContainer.has(containerNo))
            factoryTypesByContainer.set(containerNo, []);
        factoryTypesByContainer.get(containerNo).push(factoryType);
    }
    const containersWithFob = new Set();
    for (const [containerNo, types] of factoryTypesByContainer) {
        if (containerQualifiesForAllocation(types))
            containersWithFob.add(containerNo);
    }
    ...
    for (let r = headerRowIdx + 1; r < rows.length; r++) {
        ...
        const destWarehouse = destWarehouseIdx >= 0 ? String(row[destWarehouseIdx] ?? '').trim() : '';
        const remarkParts = [
...
/** 按表头自动选择体积解析器 */
export function parseVolumeSheetRows(rows) {
    if (isTransferVolumeExport(rows))
        return parseTransferVolumeSheet(rows);
    if (isEdVolumeExport(rows))
        return parseEdVolumeSheet(rows);
    return parseMerchantShipmentRows(volumeRowsToObjects(rows));
}
function volumeRowsToObjects(rows) {
    if (!rows.length)
        return [];
 ...
 */
export function parseEdVolumeSheet(rows) {
    const errors = [];
    const items = [];
    let skippedRows = 0;
    if (rows.length < 2) {
...
export function parseMerchantShipmentRows(rows) {
    const errors = [];
    const items = [];
    let skippedRows = 0;
    const hasFactoryType = rows.some((row) => (row['工厂类别'] ?? row.factory_type ?? row['是否退税'] ?? '').trim());
    const containersInFile = new Set();
    const containersWithFob = new Set();
    const factoryTypesByContainer = new Map();
    if (hasFactoryType) {
        rows.forEach((row) => {
            const containerNo = cleanContainerNo(row.container_no ||
                row.containerno ||
                row['柜号'] ||
                row['货柜号'] ||
                row['箱号'] ||
    ...
    rows.forEach((row, idx) => {
        ...
        const containerNo = cleanContainerNo(row.container_no ||
            ...
            row['临柜号'] ||
            '');
        const volume = parseAmount(row.volume_cbm || row.volume || row['体积'] || row['体积m3'] || row['体积/m3']);
