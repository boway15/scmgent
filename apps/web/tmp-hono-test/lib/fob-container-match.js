...
export function uniqueContainers(values) {
    const set = new Set();
...
export function computeContainerMatch(volumeContainers, billContainers, options) {
...
export function blocksCalculation(match) {
    return match.billOnly.length > 0;
}
export function buildBillOnlyBlockMessage(match) {
    const preview = match.billOnly.slice(0, 5).join('、');
    const suffix = match.billOnly.length > 5 ? ` 等共 ${match.billOnly.length} 个柜号` : '';
    return `账单中存在无体积数据的柜号（${preview}${suffix}），请补齐 ED 体积文件或修正账单后重试`;
}
export function buildNonFobHint(match) {
    if (!match.nonFobOnly.length)
        return null;
    const preview = match.nonFobOnly.slice(0, 5).join('、');
    const suffix = match.nonFobOnly.length > 5 ? ` 等共 ${match.nonFobOnly.length} 个柜号` : '';
    return `账单中有非 FOB 柜号（${preview}${suffix}），不参与分账`;
}
export function buildNoAllocationMessage(match) {
