export function buildContainerMerchantStats(shipments) {
    const map = new Map();
    for (const row of shipments) {
        ...
        if (!entry.merchantName && row.merchantName)
            entry.merchantName = row.merchantName;
    }
...
export function statsByContainer(stats) {
    const result = new Map();
    for (const s of stats) {
        if (!result.has(s.containerNo))
            result.set(s.containerNo, []);
