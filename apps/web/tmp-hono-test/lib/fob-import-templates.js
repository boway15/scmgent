...
export async function buildFobImportTemplate(type) {
...
function buildTemplateRows(type) {
    if (type === 'volume')
        return buildVolumeTemplateRows();
    if (type === 'trucking')
        return buildTruckingTemplateRows();
    return buildFreightTemplateRows();
}
/** 1. 体积信息：表头 + 可选示例行（亦兼容 ED 大件调拨原表导出） */
function buildVolumeTemplateRows() {
    return [['柜号', '业务编号', '主体', '体积', '工厂类别', '工厂名称']];
}
/** 2. 拖车账单：第 1 行表头，宽表费用列 */
function buildTruckingTemplateRows() {
...
function buildFreightTemplateRows() {
    const header = [
        '货柜号',
        '业务编号',
        '提单号',
