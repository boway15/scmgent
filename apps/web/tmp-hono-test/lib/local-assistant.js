...
async function resolveSkuId(skuId, skuCode) {
    if (skuId)
        return skuId;
    if (!skuCode?.trim())
        return null;
...
export async function buildSkuContext(params) {
    ...
    if (params.warehouseCode) {
        ...
        lines.push(`仓 ${params.warehouseCode}：可售 ${snap.qtyAvailable}，在途 ${snap.qtyInTransit}，本仓有效 ${snap.localEffectiveQty}`);
        ...
        for (const { warehouse } of whRows) {
            if (!isPhysicalWarehouse(warehouse))
                continue;
            const snap = await getLatestInventorySnapshot(id, warehouse);
            lines.push(`仓 ${warehouse}：本仓有效 ${snap.localEffectiveQty}（可售 ${snap.qtyAvailable} / 在途 ${snap.qtyInTransit}）`);
        }
        const inProduction = await getLatestInProductionQty(id);
        if (inProduction > 0) {
            lines.push(`SKU 在产池（未分仓）：${inProduction}`);
    ...
    if (pendingSuggestions.length) {
        ...
        for (const s of pendingSuggestions) {
            lines.push(`- 仓 ${s.warehouseCode ?? '-'}：建议 ${s.suggestedQty}，日期 ${String(s.suggestedDate).slice(0, 10)}。${s.reason ?? ''}`);
        }
    }
    ...
    if (sku.leadTimeDays)
        lines.push(`采购交期：${sku.leadTimeDays} 天`);
    if (sku.merchantCode)
        lines.push(`默认商家：${sku.merchantName ?? sku.merchantCode}`);
    return lines.join('\n');
}
export async function queryLocalAssistant(query, context) {
