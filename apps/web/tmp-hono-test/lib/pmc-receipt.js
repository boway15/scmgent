...
import { db, pmcPlans, pmcPlanItems, pmcReceipts, inventoryRecords } from '../_db/index.js';
import { getLatestInventorySnapshot } from './inventory-snapshot.js';
export async function receivePmcPlanItem(input) {
    ...
    if (nextCompleted > item.plannedQty) {
        throw new Error(`到货数量超出计划：当前已完成 ${currentCompleted}，本次 ${input.qtyReceived}，计划 ${item.plannedQty}`);
    }
