...
export const US_WAREHOUSE_CODES = ['US-WEST', 'US-SOUTH', 'US-SOUTHEAST', 'US-EAST'];
export function parseOverflowCodes(raw) {
    if (!raw?.trim())
        return [];
...
export function shouldDeferReplenishment(params) {
    if (params.warehouseEffective >= params.warehouseRop)
        return true;
    if (params.networkRop <= 0)
        return false;
    return params.networkEffective >= params.networkRop;
}
/** 按分仓日均销量占比拆分总补货量 */
export function splitQtyByDailyShare(totalQty, dailyByWarehouse) {
