...
import { db, skus, stockAlerts, reorderSuggestions, pmcPlans, purchaseDrafts, salesHistory, inventoryRecords, } from '../_db/index.js';
...
dashboardRoutes.get('/dashboard', async (c) => {
    ...
    const [sales7d] = await db
        .select({ total: sql `coalesce(sum(${salesHistory.qtySold}), 0)::int` })
        .from(salesHistory)
    ...
    const salesTrend7d = await db
        .select({
        date: sql `${salesHistory.saleDate}::text`,
        qty: sql `coalesce(sum(${salesHistory.qtySold}), 0)::int`,
    })
        .from(salesHistory)
        .where(gte(salesHistory.saleDate, fromStr))
    ...
    const [openAlerts7d] = await db
        .select({ count: sql `count(*)::int` })
        .from(stockAlerts)
        .where(and(eq(stockAlerts.isResolved, false), gte(stockAlerts.notifiedAt, sevenDaysAgo)));
    const todos = [];
    ...
    const deliverySoon = await db
        ...
        .from(pmcPlans)
        .where(and(inArray(pmcPlans.status, ['confirmed', 'in_progress']), lte(pmcPlans.deliveryDate, new Date(Date.now() + 14 * 86400000)), gte(pmcPlans.deliveryDate, new Date())))
