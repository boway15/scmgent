...
import { parseImportContent, parseXlsxBuffer, runImport, } from '../lib/import/handlers.js';
import { requireImportAccess, requireMenu } from '../lib/rbac.js';
import { assertRowCount, assertUploadFile } from '../lib/upload-guard.js';
import { createImportBatch, finalizeImportBatch, listImportBatches, } from '../lib/import/batch.js';
import { BATCH_TRACKED_IMPORT_TYPES, buildImportPreviewResponse, } from '../lib/import/preview.js';
export const importRoutes = new Hono();
const VALID_TYPES = ['skus', 'inventory', 'sales', 'safety_stock', 'pmc_plans'];
...
importRoutes.post('/import/:type/preview', requireImportAccess(), async (c) => {
    ...
    if (contentType.includes('multipart/form-data')) {
        ...
        if (!body.csv?.trim())
            return c.json({ message: 'csv content required' }, 400);
...
importRoutes.post('/import/:type', requireImportAccess(), async (c) => {
    ...
    const contentType = c.req.header('content-type') ?? '';
    async function executeImport(rows, fileName, planMeta) {
