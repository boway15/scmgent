...
import { db, inventoryRecords, skus, safetyStockConfig, warehouses, channelWarehousePrefs, merchants } from '../_db/index.js';
...
import { applyReplenishLightToRows, needsReplenishmentByInventory, normalizeReplenishLight, } from '../lib/replenish-light.js';
    ...
    for (const sku of skuRows) {
        ...
        for (const wh of whList) {
            ...
            const [cfg] = await db
                ...
                .where(and(eq(safetyStockConfig.skuId, sku.id), eq(safetyStockConfig.warehouseCode, wh.code)))
                .limit(1);
            ...
            let status = 'normal';
            if (localEffectiveQty <= 0)
                status = 'stockout';
            else if (cfg?.safetyStockQty != null && localEffectiveQty < cfg.safetyStockQty)
                status = 'danger';
            else if (cfg?.reorderPoint != null && localEffectiveQty < cfg.reorderPoint)
                status = 'alert';
            draftRows.push({
                skuId: sku.id,
                code: sku.code,
...
inventoryRoutes.get('/inventory/export', async (c) => {
    ...
    const csv = buildCsv(['warehouse', 'sku_code', 'sku_name', 'replenish_light', 'qty_available', 'qty_in_transit', 'sku_qty_in_production', 'qty_reserved', 'local_effective_qty', 'reorder_point'], rows);
...
inventoryRoutes.get('/merchants', async (c) => {
    ...
    return c.json(Array.from(map.entries())
