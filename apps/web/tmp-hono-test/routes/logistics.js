...
import { db, users, fobSettlementBatches, fobServiceProviders, fobMerchantPaymentStatus, fobMerchantShipments, fobContainerMerchantStats, fobTruckingBillItems, fobFreightBillItems, fobSettlementAllocations, fobSettlementAdjustments, fobFeeAllocationRules, resetFobFeeRuleDisplayPriorities, } from '../_db/index.js';
import { enrichContainerStats } from '../lib/fob-allocation-base-meta.js';
import { getCurrentUser } from '../lib/auth-context.js';
import { parseSenweiTruckingSheet, parseSimplifiedTruckingSheet, isSenweiTruckingSheet, isSimplifiedTruckingSheet, parseHuamaoFreightSheet, parseSimplifiedFreightSheet, isHuamaoFreightSheet, isSimplifiedFreightSheet, parseMerchantShipmentRows, parseVolumeSheetRows, sheetRowsFromBuffer, } from '../lib/fob-bill-parsers.js';
import { rowsToObjects, parseDelimitedText } from '../lib/import/parse.js';
import { aggregateMerchantVolumes, allocateFees, padMissingMerchantAllocations, reconcileAllocations, shouldPadMerchantPlaceholders, summarizeByMerchant, mergeVolumeAndTicketStats, sortAllocationRows, } from '../lib/fob-settlement.js';
import { buildNoAllocationMessage, blocksCalculation, buildBillOnlyBlockMessage, } from '../lib/fob-container-match.js';
import { statsByContainer } from '../lib/fob-container-stats.js';
import { EXCEPTION_REASON_LABEL, matchAllocationRule, } from '../lib/fob-fee-rules.js';
import { loadActiveFeeRules, rebuildContainerMerchantStats, resolveRuleForBillItem, countPendingExceptions, effectiveBillAmount, FOB_NON_FOB_MARKER, isNonFobMarkerShipment, computeMatchFromShipments, partitionVolumeShipments, } from '../lib/fob-batch-helpers.js';
...
import { buildByMerchantZipFileName, buildMerchantBillWideExportAoa, buildMerchantExportFileName, buildTotalBillWideExportAoa, buildTotalExportFileName, buildXlsxBuffer, buildZipBuffer, contentDispositionAttachment, listMerchantsForExport, sanitizeExportFileName, } from '../lib/fob-reconcile-export.js';
import { validatePaymentUpdate } from '../lib/fob-payment-status.js';
const fobMenu = requireMenu('logistics.fob_settlement');
const FOB_TEMPLATE_TYPES = new Set(['volume', 'trucking', 'freight']);
export async function nextFobBatchNo() {
    const d = new Date();
...
function mapAllocationRowForCalc(a) {
...
function inferTruckingStage(feeType) {
    if (/报关|关税|增值税|查验|清关/.test(feeType))
        return 'customs';
    return 'trucking';
}
async function buildFeeLines(batchId, settlementType) {
    if (settlementType === 'trucking') {
        ...
        return trucking.map((t) => ({
            ...
            amountCny: effectiveBillAmount(Number(t.amountCny), t.adjustedAmountCny != null ? Number(t.adjustedAmountCny) : null),
    ...
    return freight.map((f) => ({
        ...
        amountCny: effectiveBillAmount(Number(f.amountCny), f.adjustedAmountCny != null ? Number(f.adjustedAmountCny) : null),
...
async function buildMerchantStatsMap(batchId) {
    ...
    const volumeMap = aggregateMerchantVolumes(shipments.map((s) => ({
        merchantCode: s.merchantCode,
        merchantName: s.merchantName,
        containerNo: s.containerNo,
        volumeCbm: Number(s.volumeCbm),
    })));
    const ticketMap = statsByContainer(stats.map((s) => ({
        merchantCode: s.merchantCode,
        merchantName: s.merchantName,
        containerNo: s.containerNo,
        volumeCbm: Number(s.volumeCbm),
...
function mapFobBatchRow(row) {
    const { serviceProvider, ...batch } = row;
    return {
        ...batch,
        serviceProvider: serviceProvider.id != null
            ? {
...
async function getBatchWithProvider(batchId) {
    const [row] = await db
        .select({
...
async function mergeMerchantPaymentSummary(batchId, summary) {
...
logisticsRoutes.get('/logistics/fob-settlements', async (c) => {
    const rows = await db
        ...
        .leftJoin(fobServiceProviders, eq(fobSettlementBatches.serviceProviderId, fobServiceProviders.id))
...
logisticsRoutes.get('/logistics/fob-settlements/:id', async (c) => {
    ...
    const [merchantShipments, containerStats, truckingItems, freightItems, allocations] = await Promise.all([
        db.select().from(fobMerchantShipments).where(eq(fobMerchantShipments.batchId, batchId)),
        db.select().from(fobContainerMerchantStats).where(eq(fobContainerMerchantStats.batchId, batchId)),
    ...
    const billContainerNos = batch.settlementType === 'trucking'
    ...
    const fobShipmentRows = merchantShipments.filter((s) => !isNonFobMarkerShipment(s.merchantCode));
    const merchantSummary = await mergeMerchantPaymentSummary(batchId, summarizeByMerchant(allocationRowsForCalc));
    return c.json({
        ...batch,
...
logisticsRoutes.post('/logistics/fob-settlements', fobMenu, async (c) => {
    ...
    const [provider] = await db
        ...
        .limit(1);
    if (!provider)
        return c.json({ message: '服务商不存在' }, 400);
    if (!provider.isActive)
        return c.json({ message: '服务商已停用' }, 400);
...
async function getBatchOr404(batchId) {
    const [batch] = await db
        .select()
        .from(fobSettlementBatches)
        .where(eq(fobSettlementBatches.id, batchId))
...
function allocationInsertValues(batchId, row) {
    return {
        batchId,
        containerNo: row.containerNo,
        merchantCode: row.merchantCode,
...
async function syncUnbalancedAllocationPlaceholders(batchId) {
    ...
    for (const fee of feeLines) {
        ...
        if (!shouldPadMerchantPlaceholders(fee, existingRows))
            continue;
...
async function refreshBillItemExceptionFlags(batchId) {
    const batch = await getBatchOr404(batchId);
    ...
    for (const item of trucking) {
        if (item.exceptionStatus === 'confirmed' || item.exceptionStatus === 'rejected')
            continue;
        const resolved = resolveRuleForBillItem(rules, item.feeType, 'trucking', item.remark, Number(item.amountCny), item.assignedMerchantCode);
        await db
    ...
    for (const item of freight) {
        if (item.exceptionStatus === 'confirmed' || item.exceptionStatus === 'rejected')
            continue;
        const resolved = resolveRuleForBillItem(rules, item.feeType, 'freight', item.remark, Number(item.amountCny), item.assignedMerchantCode);
        await db
...
function resolveItemExceptionReason(feeType, billType, rules, remark, amountCny) {
    return matchAllocationRule(feeType, billType, rules, remark, amountCny).exceptionReason;
}
async function buildBatchReconcile(batchId) {
    const batch = await getBatchOr404(batchId);
    if (!batch) {
    ...
    return reconcileAllocations(feeLines, sortAllocationRows(allocations.map((a) => mapAllocationRow(a))).map((a) => mapAllocationRowForCalc(a)), pendingExceptions);
}
logisticsRoutes.post('/logistics/fob-settlements/:id/import/trucking', fobMenu, async (c) => {
    ...
    if (batch.status === 'confirmed')
        return c.json(confirmedBatchResponse(), 400);
    ...
    for (const item of parsed.items) {
        const resolved = resolveRuleForBillItem(rules, item.feeType, 'trucking', item.remark, item.amountCny, item.assignedMerchantCode);
...
logisticsRoutes.post('/logistics/fob-settlements/:id/import/freight', fobMenu, async (c) => {
    ...
    let exceptionCount = 0;
    for (const item of parsed.items) {
        const resolved = resolveRuleForBillItem(rules, item.feeType, 'freight', item.remark, item.amountCny, item.assignedMerchantCode);
        if (item.forceException) {
            resolved.isException = true;
            resolved.exceptionStatus = 'pending';
            resolved.allocationMethod = 'manual';
...
logisticsRoutes.post('/logistics/fob-settlements/:id/import/shipments', fobMenu, async (c) => {
    ...
    if (contentType.includes('multipart/form-data')) {
        ...
        if (name.endsWith('.xlsx') || name.endsWith('.xls')) {
            ...
            if (rows.length < 2)
                return c.json({ message: '表格为空' }, 400);
            try {
                assertRowCount(rows);
            }
            catch (err) {
        ...
        const body = await c.req.json();
        if (!body.csv?.trim())
            return c.json({ message: 'csv required' }, 400);
...
logisticsRoutes.get('/logistics/fob-settlements/:id/exceptions', fobMenu, async (c) => {
    ...
    const mapExceptionItem = (row, billType) => {
        const reason = resolveItemExceptionReason(row.feeType, billType, rules, row.remark, Number(row.amountCny));
        return {
            id: row.id,
            billType,
            containerNo: row.containerNo,
            ...
            adjustedAmountCny: row.adjustedAmountCny != null ? Number(row.adjustedAmountCny) : Number(row.amountCny),
            allocationMethod: row.allocationMethod,
...
logisticsRoutes.patch('/logistics/fob-settlements/:id/exceptions/:itemId', fobMenu, async (c) => {
    ...
    const body = await c.req.json();
    if (!body.billType)
        return c.json({ message: 'billType required' }, 400);
...
logisticsRoutes.patch('/logistics/fob-settlements/:id/container-stats/:statId', fobMenu, async (c) => {
    ...
    const [row] = await db
        ...
        .returning();
    if (!row)
        return c.json({ message: 'Stat not found' }, 404);
    return c.json(row);
});
logisticsRoutes.post('/logistics/fob-settlements/:id/calculate', fobMenu, async (c) => {
    ...
    if (pendingExceptions > 0) {
        return c.json({ message: `仍有 ${pendingExceptions} 条异常费用待审核，请先处理后再核算`, pendingExceptions }, 400);
    }
...
logisticsRoutes.get('/logistics/fob-settlements/:id/reconcile', fobMenu, async (c) => {
    ...
    const allocationRows = sortAllocationRows(allocations.map((a) => mapAllocationRow(a)));
    const reconcile = reconcileAllocations(feeLines, allocationRows.map((a) => mapAllocationRowForCalc(a)), pendingExceptions);
...
logisticsRoutes.post('/logistics/fob-settlements/:id/adjustments', fobMenu, async (c) => {
    ...
    const [allocation] = await db
        ...
        .limit(1);
    if (!allocation)
        return c.json({ message: 'Allocation not found' }, 404);
    ...
    await db.insert(fobSettlementAdjustments).values({
        ...
        originalValue: body.adjustType === 'amount'
    ...
    const reconcile = reconcileAllocations(feeLines, sortAllocationRows(allocations.map((a) => mapAllocationRow(a))).map((a) => mapAllocationRowForCalc(a)), pendingExceptions);
...
logisticsRoutes.patch('/logistics/fob-settlements/:id/merchant-payments', fobMenu, async (c) => {
    ...
    for (const update of body.updates) {
        ...
        if (!merchantCode)
            return c.json({ message: 'merchantCode required' }, 400);
...
logisticsRoutes.patch('/logistics/fob-settlements/:id', fobMenu, async (c) => {
    ...
    if (body.status === 'confirmed') {
        ...
        if (pending > 0) {
            return c.json({ message: `仍有 ${pending} 条异常费用待审核，无法确认批次`, pendingExceptions: pending }, 400);
        }
...
logisticsRoutes.post('/logistics/fob-service-providers', fobMenu, async (c) => {
    ...
    if (!code || !name)
        return c.json({ message: 'code and name required' }, 400);
...
logisticsRoutes.get('/logistics/fob-fee-rules', fobMenu, async (c) => {
    ...
    const rows = await db
        ...
        .orderBy(fobFeeAllocationRules.sourceBillType, desc(fobFeeAllocationRules.priority), fobFeeAllocationRules.feeType);
    const filtered = sourceBillType === 'trucking' || sourceBillType === 'freight'
...
logisticsRoutes.patch('/logistics/fob-fee-rules/:id', fobMenu, async (c) => {
    ...
    const matchPattern = body.matchPattern !== undefined ? body.matchPattern?.trim() || null : existing.matchPattern;
    if (!feeType && !matchPattern) {
...
async function loadReconcileExportData(batchId) {
    const batch = await getBatchWithProvider(batchId);
    if (!batch)
        return null;
    ...
    if (!provider)
        return null;
    const [allocationRows, feeRules, paymentRows, truckingItems, freightItems, containerStats] = await Promise.all([
        db
            .select()
            .from(fobSettlementAllocations)
    ...
    const paymentByMerchant = new Map(paymentRows.map((p) => [
        p.merchantCode,
        {
...
logisticsRoutes.get('/logistics/fob-settlements/:id/export/reconcile-by-merchant', fobMenu, async (c) => {
    const batchId = c.req.param('id');
    const payload = await loadReconcileExportData(batchId);
    if (!payload)
        return c.json({ message: 'Batch not found' }, 404);
    ...
    for (const merchant of merchants) {
        ...
        let filename = buildMerchantExportFileName(merchant.merchantName, merchant.merchantCode, payload.batch.settlementPeriod);
    ...
    const zipName = buildByMerchantZipFileName(payload.batch.batchNo, payload.batch.settlementPeriod);
