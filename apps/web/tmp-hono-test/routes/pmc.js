...
import { db, pmcPlans, pmcPlanItems, skus } from '../_db/index.js';
...
import { buildPmcPlanCsv, buildPmcPlanXlsx } from '../lib/pmc-export.js';
import { receivePmcPlanItem } from '../lib/pmc-receipt.js';
...
pmcRoutes.get('/pmc/plans', async (c) => {
    ...
    const rows = status
        ? await base.where(eq(pmcPlans.status, status)).orderBy(desc(pmcPlans.planDate))
...
pmcRoutes.post('/pmc/plans', requireMenu('pmc.list'), async (c) => {
    ...
    const planWarehouse = body.targetWarehouseCode?.trim() || body.items[0]?.warehouseCode?.trim();
    if (!planWarehouse) {
...
pmcRoutes.put('/pmc/plans/:id/items/:itemId', requireMenu('pmc.list'), async (c) => {
    ...
    const [row] = await db
        .update(pmcPlanItems)
        .set(body)
        .where(and(eq(pmcPlanItems.id, c.req.param('itemId')), eq(pmcPlanItems.planId, c.req.param('id'))))
        .returning();
    if (!row)
        return c.json({ message: 'Item not found' }, 404);
    return c.json(row);
});
pmcRoutes.post('/pmc/plans/:id/items/:itemId/receive', requireMenu('pmc.list'), async (c) => {
    const user = await getCurrentUser(c);
    const body = await c.req.json();
    try {
        const result = await receivePmcPlanItem({
            planId: c.req.param('id'),
            planItemId: c.req.param('itemId'),
