...
import { db, spus, merchants, skus, skuSuppliers, } from '../_db/index.js';
import { setDefaultSkuSupplier, syncSkuDefaultMerchant, } from '../lib/product-master.js';
...
productRoutes.get('/spus/:id', async (c) => {
    ...
    if (!row)
        return c.json({ message: 'SPU not found' }, 404);
...
productRoutes.put('/merchants/:id', requireMenu('data.products'), async (c) => {
    ...
    const [row] = await db
        ...
        .returning();
    if (!row)
        return c.json({ message: 'Merchant not found' }, 404);
...
productRoutes.post('/skus/:skuId/suppliers', requireMenu('data.products'), async (c) => {
    ...
    if (!merchantId)
        return c.json({ message: 'merchantId or merchantCode required' }, 400);
...
productRoutes.put('/sku-suppliers/:id/default', requireMenu('data.products'), async (c) => {
    ...
    if (!link)
        return c.json({ message: 'Supplier link not found' }, 404);
