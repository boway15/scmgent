...
function parseSalesFilters(c) {
    ...
    const conditions = [];
    if (skuCode)
        conditions.push(eq(skus.code, skuCode));
    if (from)
        conditions.push(gte(salesHistory.saleDate, from));
    if (to)
        conditions.push(lte(salesHistory.saleDate, to));
    if (channel)
        conditions.push(eq(salesHistory.channel, channel));
    if (warehouse)
        conditions.push(eq(salesHistory.warehouseCode, warehouse));
    return conditions;
}
salesRoutes.get('/sales/history', async (c) => {
    ...
    const summaryBase = db
        .select({
        totalQty: sql `coalesce(sum(${salesHistory.qtySold}), 0)::int`,
        rowCount: sql `count(*)::int`,
    })
        .from(salesHistory)
        .innerJoin(skus, eq(skus.id, salesHistory.skuId))
        .$dynamic();
...
salesRoutes.get('/sales/history/export', async (c) => {
    ...
    const csv = buildCsv(['sku_code', 'sku_name', 'sale_date', 'qty_sold', 'channel', 'warehouse_code'], rows.map((r) => [
        r.skuCode,
        r.skuName,
