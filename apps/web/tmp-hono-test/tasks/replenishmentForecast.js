...
import { db, skus, salesHistory, reorderSuggestions, safetyStockConfig, warehouses, spus } from '../_db/index.js';
import { applyMoq, calcReplenishment, resolveEffectiveMoq } from '../lib/replenishment.js';
import { getLatestInventorySnapshot, getRegionPoolSnapshot, } from '../lib/inventory-snapshot.js';
import { shouldDeferReplenishment, splitQtyByDailyShare, US_WAREHOUSE_CODES, } from '../lib/warehouse-domain.js';
import { normalizeReplenishLight, shouldReplenishByLight, } from '../lib/replenish-light.js';
...
async function upsertSafetyStock(skuId, warehouseCode, calc) {
    const [existing] = await db
        .select()
        .from(safetyStockConfig)
        .where(and(eq(safetyStockConfig.skuId, skuId), eq(safetyStockConfig.warehouseCode, warehouseCode)))
        .limit(1);
    if (existing) {
        await db
...
export async function runReplenishmentForecast() {
    const spuMoqMap = new Map((await db.select({ id: spus.id, moq: spus.moq }).from(spus)).map((s) => [s.id, s.moq]));
    ...
    for (const sku of activeSkus) {
        ...
        const effectiveMoq = resolveEffectiveMoq(sku.moq, sku.spuId ? spuMoqMap.get(sku.spuId) : null);
        ...
        for (const wh of whRows) {
            ...
            if (wh.regionGroup === 'US' && usPool.effectiveQty < usNetworkRop) {
                const usSales = salesRows
                    ...
                    ? US_WAREHOUSE_CODES.includes(s.warehouseCode)
                    : false)
                    .map((s) => ({ qtySold: s.qtySold, saleDate: String(s.saleDate) }));
            ...
            const poolNote = wh.regionGroup === 'US' ? `，US仓网合计 ${usPool.effectiveQty}/${usNetworkRop}` : '';
            const moqNote = effectiveMoq > 0 && suggestedQty > rawQty ? `，MOQ ${effectiveMoq}` : '';
