...
import { db, skus, safetyStockConfig, stockAlerts, warehouses } from '../_db/index.js';
...
export async function runStockAlert() {
    ...
    const whRegion = new Map((await db.select({ code: warehouses.code, regionGroup: warehouses.regionGroup }).from(warehouses)).map((w) => [w.code, w.regionGroup]));
    const alerts = [];
    const usPoolCache = new Map();
    const usRopCache = new Map();
    for (const row of configRows) {
        if (row.warehouseCode === 'ALL')
            continue;
        ...
        if (region === 'US') {
            if (!usPoolCache.has(row.skuId)) {
                ...
                const usConfigs = configRows.filter((r) => r.skuId === row.skuId && whRegion.get(r.warehouseCode) === 'US');
                usRopCache.set(row.skuId, usConfigs.reduce((s, r) => s + (r.reorderPoint ?? 0), 0));
            }
            const pool = usPoolCache.get(row.skuId);
            const networkRop = usRopCache.get(row.skuId) ?? 0;
            if (shouldDeferReplenishment({
        ...
        if (currentQty <= 0)
            alertType = 'stockout';
        else if (safetyQty > 0 && currentQty < safetyQty)
            alertType = 'below_safety';
    ...
    const engine = difySummary ? 'local-per-warehouse+dify-summary' : 'local-per-warehouse';
