...
export type MatrixBillScope = 'trucking' | 'freight' | 'all';

/** 前三列冻结区总宽度（px） */
const FROZEN_COL_WIDTHS = [168, 80, 88] as const;
...
function frozenStyle(colIdx: 0 | 1 | 2, isHeader: boolean): CSSProperties {
  return {
    position: 'sticky',
...
function frozenCellClass(colIdx: 0 | 1 | 2, bg: string, isHeader = false) {
  return cn(
    bg,
    isHeader && colIdx === 2 && 'shadow-[2px_0_6px_-2px_rgba(0_0_0_0.08)]',
    !isHeader && colIdx === 2 && 'shadow-[2px_0_6px_-2px_rgba(0_0_0_0.06)]',
  );
}
...
function filterChecksByScope<T extends { sourceBillType: 'trucking' | 'freight'; sourceAmountCny: number }>(
  ...
  const withAmount = checks.filter((c) => c.sourceAmountCny > 0);
  if (scope === 'all') return withAmount;
  return withAmount.filter((c) => c.sourceBillType === scope);
}
...
export function FobContainerMatrixPanel({
  ...
  editAlloc: Record<string, string>;
  onEditAmount: (id: string, value: string) => void;
  onSaveRow: (row: AllocationItem, amount?: string) => void | Promise<void>;
  onAssignFullAmount: (
  ...
  const [billScope, setBillScope] = useState<MatrixBillScope>('trucking');
  ...
  const allPrepared = useMemo(
    () => prepareGroups(groups, billScope, containerStats),
    [groups, billScope, containerStats],
  );
  ...
  const scopeLabel =
    billScope === 'trucking' ? '拖车账单' : billScope === 'freight' ? '货代账单' : '全局';
                    ...
                    <tr className={cn('bg-muted', ROW_H_PARENT)}>
                      ...
                      >
                        ...
                        >
                          ...
                          <TruncatedTip
                            tip={`柜号：${group.containerNo}\n${merchants.length} 个主体 · ${prepared.scopedChecks.length} 项费用`}
                        ...
                        <div className="mt-0.5 truncate pl-[2.125rem] text-[11px] text-text-hint">
                          {merchants.length} 主体 · {prepared.scopedChecks.length} 项费用
                        </div>
                      </td>
                      <td
                        className={cn(
                      ...
                      >
                        <div className="truncate font-numeric text-xs text-text-sub">
                          账 {scopeTotals.billTotal.toFixed(0)}
                        </div>
                        <div
                          className={cn(
                            'truncate font-numeric text-xs font-medium',
                        ...
                        return (
                          ...
                          >
                            ...
                            <td
                              ...
                              style={frozenStyle(2, false)}
                            >
                              <TruncatedTip tip={`行合计 ¥${rowSum.toFixed(2)}`} align="right">
                                {rowSum.toFixed(2)}
                              </TruncatedTip>
                            </td>
                            {globalColumns.map((col) => {
