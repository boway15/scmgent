...
export const api = {
  ...
  previewImport: (type: ImportType, body: { csv: string }) =>
    request<{ rowCount: number; headers: string[]; preview: Array<Record<string, string>> }>(
      `/api/import/${type}/preview`,
      { method: 'POST', body: JSON.stringify(body) },
    ),
  getComplianceOverview: (params?: { category?: string; status?: ComplianceStatus }) => {
    const qs = new URLSearchParams();
    if (params?.category) qs.set('category', params.category);
    if (params?.status) qs.set('status', params.status);
    const query = qs.toString();
    return request<{
      stats: { total: number; complete: number; partial: number; missing: number };
      categories: string[];
      gaps: ComplianceSkuRow[];
    }>(`/api/compliance/overview${query ? `?${query}` : ''}`);
  },
  getComplianceSkus: (params?: { category?: string; status?: string; q?: string }) => {
    const qs = new URLSearchParams();
    if (params?.category) qs.set('category', params.category);
    if (params?.status) qs.set('status', params.status);
    if (params?.q) qs.set('q', params.q);
    const query = qs.toString();
    return request<ComplianceSkuRow[]>(`/api/compliance/skus${query ? `?${query}` : ''}`);
  },
  getSkuCompliance: (skuId: string) =>
    request<{
      hsCode?: string | null;
      originCountry?: string | null;
      declaredValue?: string | null;
      ...
      batteryType?: string | null;
      isLiquid?: boolean;
    } | null>(`/api/skus/${skuId}/compliance`),
  putSkuCompliance: (
    skuId: string,
    data: {
      hsCode?: string;
  ...
  ) => request(`/api/skus/${skuId}/compliance`, { method: 'PUT', body: JSON.stringify(data) }),
  ...
  importData: (
    ...
    request<{ imported: number; errors: string[] }>(`/api/import/${type}`, {
      method: 'POST',
  ...
  getAiConfig: () =>
    request<{ mode: 'local' | 'dify'; difyEnabled: boolean }>('/api/ai/config'),
  getAiConversations: () =>
  ...
  downloadFobReconcileByMerchant: async (id: string) => {
    ...
    await downloadAttachment(res, '按公司导出.zip');
  },
};
