...
function ReconcileViewTabs({
  ...
  const tabs: Array<{ key: ReconcileViewMode; label: string; count?: number }> = [
    ...
    { key: 'merchant', label: '按主体汇总', count: merchantCount || undefined },
  ];
    ...
    <div className="space-y-2">
      {feeUnbalanced && !compact && (
        <p className="text-xs text-amber-800">
          本费用项未平账：已列出本柜全部主体，未承担方默认为 ¥0，请修改承担金额或承担主体后保存。
        </p>
      )}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b text-left text-text-sub">
            ...
            {!compact && (
              <th className="p-2 font-normal" title="该主体在本柜内的体积份额">
                体积占比
              </th>
            )}
            {!compact && (
              <th className="p-2 font-normal" title="该主体在本柜内的票数份额（每主体 1 票）">
                票数占比
              </th>
            )}
            <th className="p-2 font-normal">承担主体</th>
...
function ImportedVolumeStatsList({ stats }: { stats: FobDetail['containerStats'] }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">分摊基数（柜 + 主体）</CardTitle>
        <p className="text-sm text-text-sub">
          各柜内主体的体积与票数（每主体 1 票）；占比为柜内份额，供按体积 / 按票规则参考
        </p>
      </CardHeader>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b text-left text-text-sub">
              ...
              <th className="p-2 font-normal">主体</th>
...
const WORKFLOW_STEPS = [
  ...
  { key: 'summary' as TabKey, label: '商家汇总' },
...
function getCalculateReadiness(
  ...
  if (!imports.trucking) blockers.push('尚未导入拖车账单');
  if (!imports.freight) blockers.push('尚未导入货代账单');
...
function getImportReadiness(data: FobDetail) {
  ...
  return { volume, trucking, freight, allReady: volume && trucking && freight };
}
...
function PrerequisiteList({
  ...
  const items = [
    ...
    { ok: imports.trucking, label: '拖车账单已导入' },
    { ok: imports.freight, label: '货代账单已导入' },
    {
      ok: billOnlyOk,
      label: billOnlyOk
        ? '账单柜号均有体积数据'
...
export function FobSettlementDetailPage() {
  ...
  const importShipments = useMutation({
    ...
    onSuccess: (r) => {
      setImportMsg((m) => ({
        ...m,
        shipments: `本次导入 ${r.imported} 条、${r.containers} 柜、${r.merchants ?? '—'} 个主体，跳过 ${r.skippedRows ?? 0} 行`,
      }));
      qc.invalidateQueries({ queryKey: ['fob-settlement', id] });
    },
  });
  ...
  const adjustAllocation = useMutation({
    mutationFn: (payload: { allocationId: string; adjustType: 'amount' | 'merchant'; value: string }) =>
      api.postFobAdjustment(id, {
        ...
        reason: payload.adjustType === 'merchant' ? '人工调整承担主体' : '人工调账',
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['fob-settlement', id] });
      refetchReconcile();
  ...
  const tabs: Array<{ key: TabKey; label: string; badge?: number }> = [
    ...
    { key: 'summary', label: '商家汇总' },
  ];

  const missingImports = [
    !workflow.imports.volume && '体积信息',
    !workflow.imports.trucking && '拖车账单',
    !workflow.imports.freight && '货代账单',
  ].filter(Boolean) as string[];

  const volumeStats = workflow.imports.volume
    ? `${data.merchantShipments.length} 行明细 · ${data.containerStats?.length ?? 0} 柜+主体 · ${merchantCount} 个主体`
    : undefined;
  const truckingStats = workflow.imports.trucking ? `${data.truckingItems.length} 条费用` : undefined;
  const freightStats = workflow.imports.freight ? `${data.freightItems.length} 条费用` : undefined;
      ...
      {tab === 'import' && (
        <div className="space-y-4">
          <div className="grid gap-4 md:grid-cols-3">
            <ImportBlock
              label="1. 体积信息"
              hint="ED 大件调拨导出明细，或按模板填写；按柜+主体汇总体积，每主体 1 票"
              accept=".csv,.xlsx,.xls"
            ...
            <ImportBlock
              ...
              done={workflow.imports.trucking}
              stats={truckingStats}
              readOnly={isReadOnly}
            ...
            <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-text-sub">
                  仅拦截未在费用规则中配置的费用项（如茶水费）及备注/金额异常。已配置为「需确认」的分摊方式（如改单费、其他）请在分摊平账时指定承担主体。
                </p>
              </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left text-text-sub">
                    ...
                    <th className="p-2 font-normal">归属主体</th>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              if ((allocationMethod === 'manual' || allocationMethod === 'fixed') && !merchant) {
                                setExceptionError('固定分摊或需确认须填写归属主体');
                                return;
                              }
                              setExceptionError('');
          ...
          <Card className="border-primary/20 bg-accent/30">
            <CardHeader>
              ...
              <p className="text-sm text-text-sub">
                ...
                ；规则为「需确认」的费用各主体默认 ¥0，请在下方平账时指定承担方；调账后标记为人工调整
              </p>
            </CardHeader>
            <CardContent className="space-y-4">
                  ...
                  {!workflow.calculated ? (
                    ...
                    <>
                      <p className="text-sm text-emerald-700">
                        {importMsg.calc || '分摊核算已完成，可查看平账结果与商家汇总'}
                      </p>
                      <Button
                        size="lg"
                        className="w-full sm:w-auto"
                ...
                <Card className={cn(!reconcile.balanced && 'border-primary/40')}>
                  <CardContent className="pt-6">
                    ...
                    <p className="mt-1 text-xs text-text-hint">
                      {reconcile.balanced ? '已平账，可确认批次' : '未平账，请在下方调整承担主体或金额'}
                    </p>
            ...
            <Card className="min-w-0 max-w-full">
              <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  <p className="text-sm text-text-sub">
                    按柜查看账单总额、分摊总额与差额；展开费用项可直接调账。切换「按主体汇总」可从商家视角核对承担金额
                  </p>
                </div>
                ...
                {reconcileView === 'merchant' && (
                  <ul className="divide-y divide-border rounded-md border border-border">
                    ...
                    {!data.merchantSummary.length && (
                      <li className="px-4 py-6 text-center text-sm text-text-hint">暂无主体汇总</li>
                    )}
                  </ul>
                )}
          ...
          {workflow.calculated && (
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle className="text-base">商家汇总</CardTitle>
