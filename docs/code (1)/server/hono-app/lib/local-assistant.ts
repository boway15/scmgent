...
import { db, skus, stockAlerts, reorderSuggestions, inventoryRecords } from '../_db';
import { getLatestInventorySnapshot, getLatestInProductionQty } from './inventory-snapshot';
import { IN_PRODUCTION_WAREHOUSE, isPhysicalWarehouse } from './inventory-constants';
...
const FAQ: FaqEntry[] = [
  ...
  {
    keywords: ['合规', 'hs', '海关', '申报'],
    answer:
      '跨境合规主数据在「合规管理」独立菜单维护：HS 编码、原产国、重量尺寸、电池/液体标识。\n完整标准：HS + 重量 + 原产国均已填写。支持 CSV 批量导入。',
  },
  {
    keywords: ['销量', '导入', 'csv', '数据导入'],
    answer:
      '销量历史通过「数据中心 > 数据导入」录入，用于安全库存与补货算法。\n支持 SKU、库存、销量、安全库存、PMC 计划、合规等类型。',
  },
  {
