...
import { db, kbConversations, kbMessages } from '../_db';
import { getCurrentUser } from '../lib/auth-context';
import { queryKnowledge, isDifyEnabled } from '../integrations/dify';
import { queryLocalAssistant } from '../lib/local-assistant';
import { checkAiRateLimit } from '../lib/ai-rate-limit';
import { requireMenu } from '../lib/rbac';

export const aiRoutes = new Hono();

aiRoutes.get('/ai/config', (c) =>
  c.json({
...
aiRoutes.post('/ai/chat', requireMenu('ai.chat'), async (c) => {
  ...
  try {
    ...
    if (isDifyEnabled()) {
      const result = await queryKnowledge(body.query, user.id, difyConversationId);
      ...
      const result = await queryLocalAssistant(body.query, {
        skuId: body.skuId,
