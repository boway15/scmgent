...
import {
  ...
  isEdVolumeExport,
  sheetRowsFromBuffer,
} from '../lib/fob-bill-parsers';
import { rowsToObjects, parseDelimitedText } from '../lib/import/parse';
import {
  ...
  type FeeLine,
  type AllocationRow,
} from '../lib/fob-settlement';
import {
  buildNoAllocationMessage,
  blocksCalculation,
  buildBillOnlyBlockMessage,
} from '../lib/fob-container-match';
import { statsByContainer } from '../lib/fob-container-stats';
import {
  EXCEPTION_REASON_LABEL,
  matchAllocationRule,
  type ExceptionReason,
} from '../lib/fob-fee-rules';
import {
  loadActiveFeeRules,
...
async function buildFeeLines(batchId: string): Promise<FeeLine[]> {
...
logisticsRoutes.get('/logistics/fob-settlements/:id', async (c) => {
  ...
  return c.json({
    ...batch,
    merchantShipments: merchantShipments.filter((s) => !isNonFobMarkerShipment(s.merchantCode)),
    nonFobContainers,
    containerStats,
    truckingItems,
    freightItems,
    ...
    pendingExceptions,
    merchantSummary: summarizeByMerchant(allocationRowsForCalc),
    containerMatch,
  });
});
...
logisticsRoutes.post('/logistics/fob-settlements/:id/import/trucking', fobMenu, async (c) => {
  ...
  if (!parsed.items.length) {
    return c.json({ message: '未解析到拖车费用行', errors: parsed.errors }, 400);
  }
...
logisticsRoutes.post('/logistics/fob-settlements/:id/import/freight', fobMenu, async (c) => {
  ...
  const parsed = isHuamaoFreightSheet(rows)
    ...
    : isSimplifiedFreightSheet(rows)
      ...
      : parseHuamaoFreightSheet(rows);
  if (!parsed.items.length) {
    return c.json({ message: '未解析到货运费用行', errors: parsed.errors }, 400);
  }
...
logisticsRoutes.post('/logistics/fob-settlements/:id/import/shipments', fobMenu, async (c) => {
  ...
  if (contentType.includes('multipart/form-data')) {
    ...
    if (name.endsWith('.xlsx') || name.endsWith('.xls')) {
      ...
      parsed = isEdVolumeExport(rows)
        ? parseEdVolumeSheet(rows)
        : parseMerchantShipmentRows(
            (() => {
...
logisticsRoutes.patch('/logistics/fob-settlements/:id/exceptions/:itemId', fobMenu, async (c) => {
  ...
  if (body.exceptionStatus === 'confirmed' && resolvedMethod === 'manual') {
    ...
    if (!merchant) {
      return c.json({ message: '人工分摊须指定归属主体' }, 400);
    }
  }
...
logisticsRoutes.patch('/logistics/fob-settlements/:id/container-stats/:statId', fobMenu, async (c) => {
  ...
  if (body.ticketCount == null || body.ticketCount < 0 || body.ticketCount > 1) {
    return c.json({ message: 'ticketCount 须为 0 或 1（同一货柜内每主体最多 1 票）' }, 400);
  }
...
logisticsRoutes.post('/logistics/fob-settlements/:id/calculate', fobMenu, async (c) => {
  ...
  if (!trucking.length && !freight.length) {
    return c.json({ message: '请先导入拖车账单或货代账单' }, 400);
  }

  const containerMatch = computeMatchFromShipments(shipments, [
  ...
  const feeLines = await buildFeeLines(batchId);
  ...
  return c.json({
    ...
    merchantSummary: summarizeByMerchant(allocations),
    warnings,
    reconcile,
  });
});
  ...
  if (body.adjustType === 'exclude') {
    ...
    await db
      ...
      .set({
        ...
        allocationMethod: 'manual',
        isManualOverride: true,
        overrideReason: body.reason ?? '人工调整承担主体',
      })
      .where(eq(fobSettlementAllocations.id, body.allocationId));
  }
...
async function loadReconcileExportData(batchId: string) {
  ...
  const [allocationRows, feeRules] = await Promise.all([
    db
      .select()
      .from(fobSettlementAllocations)
  ...
  return { batch, allocations, feeRules };
}
...
logisticsRoutes.get('/logistics/fob-settlements/:id/export/reconcile-total', fobMenu, async (c) => {
  ...
  const rows = buildReconcileWideTableAoa(payload.allocations, payload.feeRules);
...
logisticsRoutes.get(
  ...
  async (c) => {
    ...
    for (const merchant of merchants) {
      const rows = buildReconcileWideTableAoa(
        payload.allocations,
        payload.feeRules,
