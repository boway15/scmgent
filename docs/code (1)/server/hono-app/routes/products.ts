...
productRoutes.get('/skus/:skuId/compliance', async (c) => {
  const [row] = await db
    .select()
    .from(skuCompliance)
    .where(eq(skuCompliance.skuId, c.req.param('skuId')))
    .limit(1);
  return c.json(row ?? null);
});

productRoutes.put('/skus/:skuId/compliance', requireMenu('compliance.skus'), async (c) => {
  const skuId = c.req.param('skuId');
  const body = await c.req.json<{
    hsCode?: string;
  ...
  const values = {
    ...
    originCountry: body.originCountry?.trim()?.toUpperCase(),
    declaredValue: body.declaredValue?.toString(),
    weightKg: body.weightKg?.toString(),
    lengthCm: body.lengthCm?.toString(),
    widthCm: body.widthCm?.toString(),
    heightCm: body.heightCm?.toString(),
    batteryType: body.batteryType?.trim(),
    isLiquid: body.isLiquid ?? false,
    updatedAt: new Date(),
  };

  const [existing] = await db.select().from(skuCompliance).where(eq(skuCompliance.skuId, skuId)).limit(1);
  if (existing) {
    const [row] = await db.update(skuCompliance).set(values).where(eq(skuCompliance.skuId, skuId)).returning();
    return c.json(row);
  }

  const [row] = await db.insert(skuCompliance).values({ skuId, ...values }).returning();
  return c.json(row, 201);
});
