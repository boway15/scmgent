...
import { pickField, rowsToObjects, parseDelimitedText } from './parse';
import { nextPlanNo } from '../../routes/procurement';
import { upsertSkuSupplierFromImport } from '../product-master';
import { parseBoolField } from '../compliance';
import { IN_PRODUCTION_WAREHOUSE } from '../inventory-constants';
import { normalizeReplenishLight, parseReplenishLight } from '../replenish-light';
...
export async function importSalesRows(rows: Array<Record<string, string>>): Promise<ImportResult> {
  let imported = 0;
  const errors: string[] = [];
...
export async function importComplianceRows(
  rows: Array<Record<string, string>>,
): Promise<ImportResult> {
  let imported = 0;
  const errors: string[] = [];
  ...
  for (const row of rows) {
    ...
    const values = {
      hsCode: pickField(row, 'hs_code') || undefined,
      originCountry: pickField(row, 'origin_country')?.toUpperCase() || undefined,
      declaredValue: pickField(row, 'declared_value') || undefined,
      weightKg: pickField(row, 'weight_kg') || undefined,
      lengthCm: pickField(row, 'length_cm') || undefined,
      widthCm: pickField(row, 'width_cm') || undefined,
      heightCm: pickField(row, 'height_cm') || undefined,
      batteryType: pickField(row, 'battery_type') || undefined,
      isLiquid: parseBoolField(pickField(row, 'is_liquid')),
      updatedAt: new Date(),
    };
    ...
    const [existing] = await db
      .select()
      .from(skuCompliance)
      .where(eq(skuCompliance.skuId, sku.id))
      .limit(1);

    if (existing) {
      await db.update(skuCompliance).set(values).where(eq(skuCompliance.skuId, sku.id));
    } else {
      await db.insert(skuCompliance).values({ skuId: sku.id, ...values });
    }
    imported++;
  }
...
export type ImportType = 'skus' | 'inventory' | 'sales' | 'safety_stock' | 'pmc_plans' | 'compliance';
...
export async function runImport(
  ...
  switch (type) {
    ...
    case 'inventory':
      return importInventoryRows(rows, userId);
    case 'sales':
      return importSalesRows(rows);
    case 'safety_stock':
      return importSafetyStockRows(rows);
    case 'pmc_plans':
      return importPmcPlanRows(rows, userId, planMeta);
    case 'compliance':
      return importComplianceRows(rows);
    default:
