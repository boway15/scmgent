...
import { db, purchaseDrafts, skus, pmcPlans } from '../_db';
    ...
    ? await baseQuery
        .where(
          and(
            ...
            eq(purchaseDrafts.status, status as 'draft' | 'submitted' | 'cancelled'),
          ),
        )
...
procurementRoutes.patch('/purchase-drafts/:id', requireMenu('pmc.tracking'), async (c) => {
  const body = await c.req.json<{ status?: 'draft' | 'submitted' | 'cancelled'; remark?: string }>();
  ...
  if (!row) return c.json({ message: 'Draft not found' }, 404);
  return c.json(row);
});
