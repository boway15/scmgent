...
import { db, roles, roleMenus, menus, users } from '../_db';
import { requireSuperAdmin } from '../middleware/auth';
import { clearMenuCache } from '../lib/rbac';
...
roleRoutes.post('/roles', requireSuperAdmin, async (c) => {
  const body = await c.req.json<{ name: string; code: string; description?: string }>();
...
roleRoutes.delete('/roles/:id', requireSuperAdmin, async (c) => {
  ...
  if (role.isSystem) return c.json({ message: 'Cannot delete system role' }, 400);
